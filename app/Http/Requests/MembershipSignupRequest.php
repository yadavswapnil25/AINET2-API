...
class MembershipSignupRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'dob'               => 'required_if:type,Individual|string|before:today',
            'gender'            => ['required_if:type,Individual', 'string', new Enum(GenderEnum::class)],
            'mobile'            => 'required|string|max:20',
            'whatsapp_no'       => 'required|string|max:20',
            'email'             => 'required|email|unique:users,email',
            'pin'               => 'required_if:type,Individual|string|max:10',
            'address'           => 'required|string|max:500',
            'state'             => 'required|string|max:100',
            'district'          => 'required|string|max:100',
            'teaching_exp'      => 'required_if:type,Individual|integer|min:0',
            'qualification'     => ['required_if:type,Individual', 'array', 'min:1'],
            'qualification.*'   => ['string', 'max:255'],
            'area_of_work'      =>  ['required_if:type,Individual', 'array', 'min:1'],
            'area_of_work.*'    => ['string'],
